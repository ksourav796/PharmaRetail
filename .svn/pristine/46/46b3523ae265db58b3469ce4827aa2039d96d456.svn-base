...
app.controller('stateCtrl',
    function ($scope, $http, $location, $filter, $timeout, Notification) {// body...
        ...
        $scope.inactiveState = function () {
            ...
            $scope.getStateList();
            // $http.post('/pos' + "/getStateList?type=" + $scope.inactiveStatus).then(function (response) {
        ...
        $scope.getStateList = function (type, page) {
            ...
            $http.post('/pos' + '/getPaginatedStateList?type=' + $scope.inactiveStatus + '&searchText=' + $scope.stateSearchText, angular.toJson(paginationDetails)).then(function (response) {
                var data = response.data;
                console.log(data);
        ...
        $scope.getCountryList = function () {
            $http.post('/pos' + '/getCountryList').then(function (response) {
                var data = response.data;
        ...
        $scope.editState = function (name) {
            $http.post('/pos' + '/editState?stateName=' + name).then(function (response) {
                var data = response.data;
        ...
        $scope.saveState = function () {
            if (angular.isUndefined($scope.countryId)||$scope.countryId=== "") {
                Notification.warning({message: 'Country can not be empty', positionX: 'center', delay: 2000});
            }
            else if (angular.isUndefined($scope.stateNameText)||$scope.stateNameText === "") {
                ...
                $http.post('/pos' + '/saveState', angular.toJson(saveStateDetails, "Create")).then(function (response, status, headers, config) {
                    var data = response.data;
                    if (data == "") {
        ...
        $scope.deleteState = function (data) {
            bootbox.confirm({
                ...
                callback: function (result) {
                    if (result == true) {
                        $scope.id = data.id;
                        $http.post('/pos' + '/deleteState?id=' + $scope.id).then(function (response, status, headers, config) {
                            var data = response.data;
