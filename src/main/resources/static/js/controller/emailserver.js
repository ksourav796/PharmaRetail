...
app.controller('emailserverCtrl',
    function ($scope, $http, $location, $filter, Notification, ngTableParams, $timeout, $window) {
        ...
        $scope.getMailList = function () {
            $http.post('/pos' + '/getPaginatedMailList',{
                params: {
        ...
        $scope.editMail = function(data) {
            $http.post('/pos' + '/editMail?userName=' + data.userName).then(function (response) {
        ...
        $scope.deleteMail = function (data) {
            bootbox.confirm({
                ...
                callback: function (result) {
                    //  alert("delete");
                    if(result == true){
                        $http.post("/pos" + '/deleteMail?userName=' +data.userName).then(function (response, status, headers, config) {
                            var data = response.data;
                            if(data=="") {
                                $scope.getMailList();
        ...
        $scope.saveMail = function () {
            ...
            if(mailId==true){
                ...
                $http.post("/pos" + "/"  + '/saveMail', angular.toJson(saveMailDetails, "Create")).then(function (response, status, headers, config) {
                    var data = response.data;
                    if(data==""){
